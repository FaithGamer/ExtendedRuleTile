...
public class ExtendedRuleTile<T> : ExtendedRuleTile
{
    /// <summary>
...
/// </summary>
[Serializable]
[CreateAssetMenu(menuName = "Extended Rule Tile")]
public class ExtendedRuleTile : TileBase
{
    /// <summary>
    ...
    /// <summary>
    /// Angle in which the ExtendedRuleTile is rotated by for matching in Degrees.
    /// </summary>
    public virtual int m_RotationAngle => 90;

    /// <summary>
    /// Number of rotations the ExtendedRuleTile can be rotated by for matching.
    /// </summary>
    ...
    public class TilingRuleOutput
    {
        ...
        public Vector2Int m_ExtendSize = Vector2Int.one;
    ...
    public class TilingRule : TilingRuleOutput
    {
        ...
        /// <summary>
        /// * Preset this list to ExtendedRuleTile backward compatible, but not support for HexagonalExtendedRuleTile backward compatible.
        /// </summary>
    ...
    /// </summary>
    [HideInInspector] public List<TilingRule> m_TilingRules = new List<ExtendedRuleTile.TilingRule>();

    /// <summary>
    /// Returns a set of neighboring positions for this ExtendedRuleTile
    /// </summary>
    ...
    /// <summary>
    /// Updates the neighboring positions of this ExtendedRuleTile
    /// </summary>
        ...
        foreach (TilingRule rule in m_TilingRules)
        {
            if (RuleMatches(rule, position, tilemap, ref transform))
            {
                switch (rule.m_Output)
                {
                    ...
                    case TilingRuleOutput.OutputSprite.Extended:
                        // https://github.com/Vinark117/Tutorials/blob/main/CustomScriptedTiles/VariableTile.cs
                        if (rule.m_Sprites.Length != rule.m_ExtendSize.x * rule.m_ExtendSize.y)
                        {
                            Debug.LogWarning("ExtendedTilingRule: numbers of sprite doesn't match the extend size");
                            break;
                        }

                        //prevents the values to be negative
                        while (position.x < rule.m_ExtendSize.x)
                        {
                            position.x += rule.m_ExtendSize.x;
                        }
                        while (position.y < rule.m_ExtendSize.y)
                        {
                            position.y += rule.m_ExtendSize.y;
                        }

                        //get the index on each axis
                        int x = position.x % rule.m_ExtendSize.x;
                        int y = position.y % rule.m_ExtendSize.y;

                        //get the index in the array
                        int spriteIndex = x + (((rule.m_ExtendSize.y - 1) * rule.m_ExtendSize.x) - y * rule.m_ExtendSize.x);

                        //returns the correct sprite
                        tileData.sprite = rule.m_Sprites[spriteIndex];
                        break;
                }
    ...
    static KeyValuePair<HashSet<TileBase>, HashSet<Vector3Int>> CachingTilemapNeighborPositions(Tilemap tilemap)
    {
        ...
        for (int i = 0; i < usedTileCount; i++)
        {
            ...
            usedTiles.Add(tile);
            ExtendedRuleTile ruleTile = null;

            if (tile is ExtendedRuleTile rt)
                ruleTile = rt;
    ...
    public override void RefreshTile(Vector3Int position, ITilemap tilemap)
    {
        ...
        var neighborPositionsExtendedRuleTile = neighborPositionsSet.Value;
        foreach (Vector3Int offset in neighborPositionsExtendedRuleTile)
        {
    ...
    /// <summary>
    /// Returns custom fields for this ExtendedRuleTile
    /// </summary>
    /// <param name="isOverrideInstance">Whether override fields are returned</param>
    /// <returns>Custom fields for this ExtendedRuleTile</returns>
    public FieldInfo[] GetCustomFields(bool isOverrideInstance)
    {
        return this.GetType().GetFields(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
            .Where(field => typeof(ExtendedRuleTile).GetField(field.Name) == null)
